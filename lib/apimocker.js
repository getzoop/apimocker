    ...
    apiMocker.middlewares.push(multer({
      storage: multer.diskStorage({
        ...
        filename: options.useUploadFieldname
          ? (req, filename, cb) => { cb(null, filename.fieldname); }
          : (req, filename, cb) => { cb(null, filename.originalname); },
      }),
    }).any());
  }
...
apiMocker.sendResponse = (req, res, serviceKeys) => {
  ...
  // Change latency based on switch
  if (options.switch) {
    const optionsClone = _.clone(options);
    apiMocker.setSwitchOptions(optionsClone, req);
    latency = optionsClone.latency;
  }
...
const getRequestParam = (req, key) => {
  const rawParamValue = req.body[key]
    || req.params[key]
    || req.query[key]
    || req.header(key);
